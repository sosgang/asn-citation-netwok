		...
		for sector in conf.sectors:
			...
			query2 = '''SELECT authorId
				...
				WHERE 
				 ...
				 ORDER BY authorId'''
			cur = conn.cursor()
			rows = cur.execute(query2, (sector,sector))
			for row in rows:
				authorId = row[0]
				cur = conn.cursor()
				cur.execute("SELECT id FROM authorScopus WHERE id = ?", (authorId,))
				rows2 = cur.fetchall()
				...
				print (authorId)
				print (rows2)
				#print (query3.format(settore=sector,idaut=rows2[0][0],givenname=rows2[0][1],surname=rows2[0][2],initials=rows2[0][3],orcid=rows2[0][3]))
